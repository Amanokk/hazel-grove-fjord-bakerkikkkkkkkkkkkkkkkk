...
export function ApontamentoForm({
  ...
  return (
    <div className="flex flex-col gap-3 pb-28">
      {openHint ? (
        <div className="rounded-xl border border-ok/30 bg-ok-fg px-4 py-3 text-sm text-ok">
          Esta máquina já tem uma atividade em andamento. Salvar atualiza o mesmo registro para toda a
          equipe. Ela só termina quando você tocar em Encerrar.
        </div>
      ) : (
        <p className="text-xs text-muted">
          Nada é criado sozinho. A atividade só entra no diário quando você tocar em Concluir, e fica
          aberta até Encerrar.
        </p>
      )}
      ...
      >
        ...
        <ChoiceList
          ...
          onChange={(id) => {
            ...
            set({ equipmentId: id, activityId: keep ? draft.activityId : "" });
            setOpen("act");
          }}
        />
      </Step>
      ...
      <p className="text-xs text-muted">
        {work ? `${work.code} ${work.name}` : "Sem obra"} · save global: a equipe vê na hora, até Encerrar
      </p>
      ...
      <div className="no-print fixed inset-x-0 bottom-0 z-20 mx-auto max-w-lg border-t border-border bg-surface/95 p-3 pb-[max(12px,env(safe-area-inset-bottom))] backdrop-blur-sm">
        ...
        {openHint && onEncerrar ? (
          <Button className="mt-2 w-full" size="lg" variant="outline" onClick={onEncerrar}>
            Encerrar atividade
          </Button>
        ) : null}
      </div>
