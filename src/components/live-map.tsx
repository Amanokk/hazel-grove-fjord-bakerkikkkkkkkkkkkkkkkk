...
import { DEFAULT_CENTER, haversineMeters, projectMercator } from "@/lib/geo";
...
export function collectMarkers(opts: {
  ...
  const coveredOpen = new Set<string>();
  ...
  for (const p of opts.presence) {
    ...
    const a = opts.apontamentos.find(
      (x) => !x.end && (x.id === p.apontamentoId || x.equipmentId === p.equipmentId || x.deviceId === p.deviceId),
    );
...
export function LiveMap({
  ...
  const center = useMemo(() => {
    ...
    if (markers.length > 0) {
      ...
      if (me) target = { lat: me.lat, lng: me.lng };
      else {
        target = {
