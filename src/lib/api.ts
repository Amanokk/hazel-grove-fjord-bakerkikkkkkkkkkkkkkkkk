...
export const upsertApontamento = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<Apontamento> => {
    ...
    const [existingById] = await sql<{ id: string; start_time: string }>`
    ...
    if (!existingById) {
      const [openRow] = await sql<{ id: string; start_time: string }>`
    ...
    const [row] = await sql<AptRow>`select * from apontamentos where id = ${id}`;
    return mapApt(row);
  });
...
const presenceSchema = z.object({
  ...
  apontamentoId: z.string().nullable(),
});
...
export const reverseGeocode = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const lng = Number(data.lng.toFixed(4));
