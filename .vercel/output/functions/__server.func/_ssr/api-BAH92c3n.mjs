...
//#region node_modules/.nitro/vite/services/ssr/assets/api-BAH92c3n.js
...
async function createPgliteSql() {
	...
	const migrate = async () => {
		const migrations = /* #__PURE__ */ Object.assign({
			...
			"/migrations/0003_global_open.sql": _0003_global_open_default
		});
...
function mapPresence(r) {
	return {
		...
		apontamentoId: r.apontamento_id ?? null,
		updatedAt: iso(r.updated_at)
	};
}
...
var getSnapshot = createServerFn({ method: "GET" }).handler(getSnapshot_createServerFn_handler, async () => {
	...
	const [works, streets, equipment, activities, apontamentos, presence] = await Promise.all([
		...
		sql`select * from presence where updated_at > now() - interval '8 minutes'`
	]);
	return {
...
var upsertApontamento = createServerFn({ method: "POST" }).validator(saveSchema).handler(upsertApontamento_createServerFn_handler, async ({ data }) => {
	...
	const [existingById] = await sql`
      select id, start_time from apontamentos where id = ${data.id}`;
	let id = data.id;
	let start = data.start;
	if (!existingById) {
		const [openRow] = await sql`
        select id, start_time from apontamentos
        where equipment_id = ${data.equipmentId} and end_time is null
        limit 1`;
		if (openRow) {
			id = openRow.id;
			start = openRow.start_time;
		}
	} else if (!data.end) await sql`update apontamentos
	...
	await sql`insert into apontamentos (
      ...
      ${id}, ${data.date}::date, ${start}, ${data.end}, ${data.workId}, ${workName},
    ...
    on conflict (id) do update set
      ...
      updated_at = now()`;
	const [row] = await sql`select * from apontamentos where id = ${id}`;
	return mapApt(row);
});
...
var getPresence_createServerFn_handler = createServerRpc({
	id: "00292b52dd7688df04a75490ecb496222dc0fa08d0395f0171ed923baec56cf8",
	name: "getPresence",
	filename: "src/lib/api.ts"
}, (opts) => getPresence.__executeServer(opts));
var getPresence = createServerFn({ method: "GET" }).handler(getPresence_createServerFn_handler, async () => {
	return (await (await getSql())`select * from presence where updated_at > now() - interval '8 minutes'`).map(mapPresence);
});
...
var reverseGeocode = createServerFn({ method: "POST" }).validator(object({
	...
	const url = `https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${Number(data.lat.toFixed(4))}&lon=${Number(data.lng.toFixed(4))}&zoom=18&addressdetails=1`;
	try {
